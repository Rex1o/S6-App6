...
def main():
    ...
    query = """
    SELECT * FROM device_client_info ORDER BY time DESC
    """
    cursor.execute(query)
    data = cursor.fetchall()

    # Récupérer les noms des colonnes
    cursor.execute("PRAGMA table_info(device_client_info)")
    ...
    st.title("Données d'entrée-sortie de la base de données assurance")
    st.write(df)
